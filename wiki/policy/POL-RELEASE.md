...
- **수동 OTA(production)**: 사용자에게 닿는 발행. `release/<버전>` 가지에서 수동 실행으로만 낸다 — 다른 가지에서 시도하면 워크플로가 거부한다([`ADR-0021`](../decisions/ADR-0021-release-strategy.md)).
...
- 스토어 새 버전 안내도 같다([`RULE-OTA-010`](../rules/release.md)) — **닫을 수 있고**, 오프라인 기기에서는 아예 나타나지 않는다. 강제 업데이트를 두지 않은 근거가 여기다.

하위 규칙: [`RULE-OTA-001`](../rules/release.md) · [`RULE-OTA-002`](../rules/release.md) · [`RULE-OTA-010`](../rules/release.md)
