...
**본문 첫 칸에 `Closes #12`를 적는다.** 템플릿이 그 줄을 미리 깔아 두는 것은 **이슈를 손으로 닫지 않기 위해서다** — 병합과 동시에 닫히지 않는 이슈는 곧 열린 채로 잊힌다([3절](#3-이슈)). 이 PR로 끝나지 않는 이슈면 `Refs #12`로 바꿔 쓰고, 닿는 이슈가 없으면 줄을 지운다. **`Refs`가 기본값이 아니다** — 이슈를 끝내는 PR인데 `Refs`로 적으면 아무 일도 일어나지 않는다.

⚠️ **`gh pr create --body`로 열면 템플릿이 적용되지 않는다.** 본문을 직접 쓸 때도 `Closes #12` 한 줄은 같이 넣는다. 백틱으로 감싼 `` `#12` ``는 링크도 트리거도 아니다.
...
```bash
git fetch origin release/1.0.2
git tag v1.0.2 origin/release/1.0.2
...
발행 전에 **번들에 담아도 되는 변경인지** 확인한다 — 스키마를 파괴적으로 바꾸거나 새 블록 타입을 쓰는 번들은 OTA로 내보내지 않는다([`rules/release.md`](rules/release.md)의 RULE-OTA-007 ~ RULE-OTA-009). 기기는 중간 번들을 건너뛰고 최신 하나만 받으며, 되돌아갈 수 있는 곳은 한 칸뿐이다.
...
production 발행이 성공하면 그때 추가된 `OTA_RELEASE`와 핫픽스도 [임시 가지 방식](#역머지는-임시-가지로-한다)으로 다시 `main`에 역머지한다. **릴리스 가지에 새 PR이 병합될 때마다 역머지 의무가 다시 생기며, 해당 배포나 핫픽스는 역머지 PR이 병합되어야 끝난다.**
