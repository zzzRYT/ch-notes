...
아래 개수는 **2026-09-07 기준의 사본**이다. 최신값은 [`../index.md`](../index.md)의 "ADR — 결정 기록" 표에서 근거 열을 세면 나온다 (커버리지 절의 `확인 필요`는 RULE까지 포함한 위키 전체 합계라 이 표와 수가 다르다).

| `confidence` | 뜻 | 개수 |
|---|---|---|
| `기록됨` | 결정이 문서·커밋에 실제로 남아 있다 | 17 |
...
[ADR-0016](ADR-0016-cold-launch-apply.md)의 이유는 **코드 주석에 문장으로 남아 있다** — 사후 재구성이 아니라 그 자리에서 쓰인 근거다. [ADR-0013](ADR-0013-release-path.md)의 워크플로 주석과 함께, 이 위키가 지어내지 않고 인용할 수 있었던 몇 안 되는 자리다.
...
| [ADR-0017](ADR-0017-commit-convention.md) | 커밋은 gitmoji를 붙인 Conventional Commits로 쓴다 | 커밋 메시지를 쓸 때 | 기록됨 |
| [ADR-0018](ADR-0018-pr-gate.md) | main은 PR과 CI를 통과한 것만 받는다 | PR을 열거나 병합 방식을 바꿀 때 | 기록됨 |
...
| [ADR-0022](ADR-0022-fsd-ddd-architecture.md) | FSD 레이어 + 선택적 DDD | 폴더를 옮기거나 새 Slice·의존성을 만들 때 | 기록됨 |
...
구조 축: [ADR-0022](ADR-0022-fsd-ddd-architecture.md). 새 폴더·Slice·공개 인터페이스를 만들기 전에 읽는다.

## 새 ADR을 쓸 때

**결정을 내린 그때 쓴다.** 초기 ADR 다수의 이유가 비어 있거나 정황 추정인 것은, 전부 나중에 복원하려 했기 때문이다. 나중에는 아무도 기억하지 못한다.
