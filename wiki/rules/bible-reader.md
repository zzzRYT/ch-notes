...
implemented_by:
  - apps/ch-life/src/widgets/scripture-browser/ui/BibleReader.tsx
  - apps/ch-life/src/widgets/scripture-browser/ui/ChapterGrid.tsx
  - apps/ch-life/src/widgets/scripture-browser/ui/VerseList.tsx
verified_by:
  - test: apps/ch-life/src/entities/scripture/api/__tests__/books-meta.test.ts#구약 39권 / 신약 27권
  - manual: 책 → 장 → 절 → 뒤로 이동
confidence: 기록됨
source:
...
implemented_by:
  - apps/ch-life/src/widgets/scripture-browser/lib/browser-search.ts
  - apps/ch-life/src/widgets/scripture-browser/lib/level-from-ref.ts
verified_by:
  - test: apps/ch-life/src/widgets/scripture-browser/lib/__tests__/browser-search.test.ts
...
implemented_by:
  - apps/ch-life/src/pages/bible-reader/ui/BibleReaderPage.tsx (insertMode="none")
  - apps/ch-life/src/widgets/scripture-browser/ui/VerseList.tsx (InsertMode)
...
`newNote` 모드는 2026-09-20 FSD 전환에서 타입에서 지웠다. `InsertMode`는 이제 `"currentNote" | "none"` 둘이다.
...
implemented_by:
  - apps/ch-life/src/entities/scripture/api/books-meta.ts (chapterCount)
  - apps/ch-life/src/widgets/scripture-browser/ui/VerseList.tsx
verified_by:
...
삽입 위치는 커서 자리가 아니라 **본문 맨 끝**이다(인용 + 빈 문단을 덧붙인다 — `features/scripture/insert`의 `insertVerse`, 호출은 `widgets/note-editor`의 `useNoteDraft().insertRef`). v1 spec 4.3의 "현재 커서 위치 삽입"은 구현되지 않았다.
