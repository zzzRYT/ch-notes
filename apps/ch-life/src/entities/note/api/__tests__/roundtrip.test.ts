...
import { noteToMarkdown } from "../markdown-serialize";
...
describe("markdown roundtrip", () => {
  it("DB → MD → DB가 본질 데이터를 보존한다", () => {
    ...
    const back = markdownToNote(md, { resolveRef: parseRef });
  ...
  it("마크다운의 (KRV) 인용은 번들 판본 editionId로 읽힌다", () => {
    const md = "> **창 1:1** (KRV)\n> 태초에 하나님이 천지를 창조하시니라\n";
    const back = markdownToNote(md, { resolveRef: parseRef });
    expect(back?.body[0]).toMatchObject({
      type: "quote",
      ref: "창 1:1",
