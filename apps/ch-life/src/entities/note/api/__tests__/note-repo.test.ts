...
import { makeSqliteNoteRepo } from "../sqlite-note-repo";
...
function setup() {
  ...
  return makeSqliteNoteRepo(adapter);
}
...
describe("note-repo", () => {
  ...
  it("import 경로: 파싱한 노트의 메타가 create 후 보존된다", async () => {
    ...
    const md =
      ...
      `location: 본당\nscripture: 요한복음 3:16\nschemaVersion: 1\n---\n\n본문\n`;
    const parsed = markdownToNote(md, { resolveRef: parseRef });
  ...
  it("editionId 없이 저장된 인용은 읽을 때 번들 판본으로 채워진다", async () => {
    const repo = setup();
    const id = await repo.create({
      title: null,
      body: [
        // editionId가 생기기 전 앱이 저장한 모양 그대로.
        {
          type: "quote",
          ref: "창 1:1",
    ...
    expect(note?.body[1]).toEqual({ type: "paragraph", text: "" });
  });

  it("editionId가 있는 인용은 그대로 보존된다", async () => {
    const repo = setup();
    const id = await repo.create({
      title: null,
      body: [
